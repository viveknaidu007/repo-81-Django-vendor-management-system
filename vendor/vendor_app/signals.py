...
"""
The signal given below will triggers performanc calculation and updates in vendor and Historical performance model 
whenever changes made in Purchase order model.

"""

@receiver(post_save, sender=PurchaseOrder)
def update_performace(sender, instance, **kwargs):
    ...
    avg_quality_rating = total_rating.get('avg_quality_rating', 0.0)
    ...
    issued_puos_count = PurchaseOrder.objects.filter(vendor=vendor).count()
    fulfillment_rate = total_completed_pos / \
        issued_puos_count*100 if issued_puos_count else 0.0
